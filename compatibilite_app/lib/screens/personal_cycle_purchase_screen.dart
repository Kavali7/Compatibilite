/// Personal Cycle Purchase Screen
/// Écran d'achat du service Cycle Personnel (Service 02)
library;
...
/// Écran d'achat du Cycle Personnel
class PersonalCyclePurchaseScreen extends StatefulWidget {
  final PricingPlan plan;

  const PersonalCyclePurchaseScreen({
    super.key,
    required this.plan,
  });

  @override
  State<PersonalCyclePurchaseScreen> createState() => _PersonalCyclePurchaseScreenState();
}

class _PersonalCyclePurchaseScreenState extends State<PersonalCyclePurchaseScreen> {
  ...
  Widget _buildIntroCard() {
    return Container(
      ...
      child: Column(
        children: [
          const Icon(Icons.calendar_month, color: Colors.teal, size: 48),
          const SizedBox(height: 16),
          Text(
            'Votre Cycle Annuel Personnel',
            style: GoogleFonts.philosopher(
              fontSize: 22,
          ...
          Text(
            'Découvrez les phases énergétiques uniques de votre année, chacune avec ses influences et conseils personnalisés pour optimiser votre vie.',
            style: TextStyle(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          _buildFeatureItem(Icons.timeline, 'Calendrier de vos périodes annuelles'),
          _buildFeatureItem(Icons.favorite, 'Domaines favorables pour chaque période'),
          _buildFeatureItem(Icons.block, 'Domaines à éviter ou reporter'),
          _buildFeatureItem(Icons.lightbulb, 'Conseils pratiques personnalisés'),
          _buildFeatureItem(Icons.format_quote, 'Affirmations positives de période'),
          _buildFeatureItem(Icons.school, 'Enseignements pour votre développement'),
          _buildFeatureItem(Icons.all_inclusive, 'Accès illimité pendant 1 an'),
        ],
      ),
    );
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            'Important pour calculer votre cycle personnel',
            style: TextStyle(
              color: AppColors.textMuted,
              fontSize: 12,
            ),
  ...
  Future<void> _processPayment() async {
    ...
    try {
      ...
      final product = Product(
        ...
        description: widget.plan.description ?? 'Abonnement Cycle Personnel 1 an',
      ...
      PaymentManager.instance.processPurchaseWithCallback(
        ...
        callback: (success, purchase, error) async {
          debugPrint('>>> Cycle Personnel payment callback: success=$success, error=$error');
          ...
          if (success && purchase != null) {
            ...
            try {
              // Créer l'abonnement
              await PersonalCycleService.instance.createSubscription(
